                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row align-items-center">
                            <div class="col-sm-6">
                                <h3 class="card-title">{{ __('Rate Plan List') }}</h3>
                            </div>
                            {{-- <span class="badge bg-warning total-count">{{ $grades->total() }}</span> --}}
                            <div class="col-sm-6">
                                @if (auth()->user()->can('rate.create'))                                    
                                <a class="btn btn-primary float-right" href="{{ route('admin.rate_plan.create',['room_id' => $room_id]) }}">
                    ...
                    {{-- table --}}
                    @include('backends.rate-plan._table')
...
<script>
    ...
    $('input.status').on('change', function () {
        ...
        $.ajax({
            type: "get",
            url: "{{ route('admin.slider.update_status') }}",
