...
<div class="modal-dialog modal-md modal-dialog-centered">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">{{ __('New Page') }}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span>
            </button>
        </div>
        <form action="{{ route('admin.pages.store') }}" class="submit-form" method="post">
                                ...
                                @if ($lang['status'] == 1)
                                    <div class="tab-pane fade {{ $lang['code'] == $default_lang ? 'show active' : '' }}" id="lang_{{ $lang['code'] }}" role="tabpanel" aria-labelledby="lang_{{ $lang['code'] }}-tab">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="title[]" id="title_{{ $lang['code'] }}" class="form-control" {{ $lang['code'] == 'en' ? 'required' : '' }}>
                                        </div>
                                    </div>
                                @endif
                            @endforeach
