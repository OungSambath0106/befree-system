...
<script>
    $(document).on('change', '.available_toggle', function () {
        if ($(this).is(':checked')) {
            ...
            $('.number_div').removeClass('d-none');
        } else {
            $('.price_div').addClass('d-none');
            $('.number_div').addClass('d-none');
        }
    })
    ...
    $(document).ready(function (e) {
        ...
        calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            firstDay: 1,
            events:{
                url:"{{route('admin.room.allotment_calendar.load_dates')}}",
                extraParams:{
                    // id:lastId,
                    id: `{{ request('room_id') }}`,
                }
    ...
    formModal = new Vue({
        ...
        methods:{
            ...
            saveForm:function () {
                this.form.target_id = `{{ request('room_id') }}`;
                var me = this;
                ...
                $.ajax({
                    url:'{{route('admin.room.allotment_calendar.store')}}',
                    data:this.form,
                    dataType:'json',
                    method:'post',
