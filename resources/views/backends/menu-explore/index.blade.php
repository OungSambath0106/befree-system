                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row align-items-center">
                            <div class="col-sm-6">
                                <h3 class="card-title">{{ __('Menu Explore List') }}</h3>
                            </div>
                            {{-- <span class="badge bg-warning total-count">{{ $grades->total() }}</span> --}}
                            <div class="col-sm-6">
                                {{-- @if (auth()->user()->can('menu.explore.create'))
                                <a class="btn btn-primary btn-modal float-right" href="#" data-href="{{ route('admin.explore_menu.create') }}" data-toggle="modal" data-container=".modal_form">
                    ...
                    {{-- table --}}
                    @include('backends.menu-explore._table')
...
<script>
    ...
    $('input.status').on('change', function () {
        ...
        $.ajax({
            type: "get",
            url: "{{ route('admin.explore_menu.update_status') }}",
