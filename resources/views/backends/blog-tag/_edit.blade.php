...
<div class="modal-dialog modal-md modal-dialog-centered">
    <div class="modal-content">
        ...
        <form action="{{ route('admin.blog-tag.update', $tag->id) }}" class="submit-form" method="post">
                                ...
                                @if ($lang['status'] == 1)
                                    ...
                                    <div class="tab-pane fade {{ $lang['code'] == $default_lang ? 'show active' : '' }}" id="lang_{{ $lang['code'] }}" role="tabpanel" aria-labelledby="lang_{{ $lang['code'] }}-tab">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="title[]" id="title_{{ $lang['code'] }}" class="form-control" value="{{ $translate[$lang['code']]['name']??$tag['title'] }}" {{ $lang['code'] == 'en' ? 'required' : '' }}>
                                        </div>
                                    </div>
                                @endif
                            @endforeach
